...
impl PatternWeight {
    ...
    pub fn get_count(&self) -> OrderedFloat<f64> {
        self.count
    }
    ...
    pub fn out_extend_iter(&self) -> DynIter<Approach> {
        let mut out_extend_codes: Vec<&Vec<u8>> = self.out_extend_map.keys().collect();
    ...
    pub fn set_count(&mut self, count: OrderedFloat<f64>) {
        self.count = count
    }
...
impl ExtendWeight {
    ...
    pub fn get_adjacency_count(&self) -> OrderedFloat<f64> {
        self.adjacency_count
    }

    pub fn get_intersect_count(&self) -> OrderedFloat<f64> {
    ...
    pub fn set_adjacency_count(&mut self, count: OrderedFloat<f64>) {
        self.adjacency_count = count
    }

    pub fn set_intersect_count(&mut self, count: OrderedFloat<f64>) {
...
impl ApproachWeight {
    pub fn is_extend(&self) -> bool {
        matches!(self, ApproachWeight::ExtendStep(_))
    }

    pub fn is_join(&self) -> bool {
        matches!(self, ApproachWeight::BinaryJoinStep(_))
    }
...
impl Catalogue {
    ...
    pub fn set_pattern_count(&mut self, pattern: &Pattern, count: OrderedFloat<f64>) -> bool {
        if let Some(index) = self.get_pattern_index(&pattern.encode_to()) {
            self.set_pattern_count_with_index(index, count);
            true
        } else {
            false
        }
    ...
    pub fn update_catalog_by_pattern(&mut self, pattern: &Pattern) {
        match self.plan_space {
            PatMatPlanSpace::ExtendWithIntersection => {
                ...
                info!("Extend Pattern Num: {}", self.get_patterns_num());
                info!("ExtendStep Num: {}", self.get_approaches_num());
            }
            PatMatPlanSpace::BinaryJoin => {
                self.update_join_steps_by_pattern(pattern);
                info!("Number of patterns after updating join steps: {}", self.get_patterns_num());
                info!("Number of approaches after updating join steps: {}", self.get_approaches_num());
            }
            PatMatPlanSpace::Hybrid => {
                self.update_extend_steps_by_pattern(pattern);
                info!("Number of patterns after updating extend steps: {}", self.get_patterns_num());
                info!("Number of approaches after updating extend steps: {}", self.get_approaches_num());
                self.update_join_steps_by_pattern(pattern);
                info!("Number of patterns after updating join steps: {}", self.get_patterns_num());
                info!("Number of approaches after updating join steps: {}", self.get_approaches_num());
            }
...
fn new_extend_step_from_adjacencies(
    pre_pattern: &Pattern, pattern: &Pattern, adjacencies: &[&Adjacency], adj_vertex_id: PatternId,
) -> ExtendStep {
...
fn new_pattern_edges_from_adjacencies<'a>(
    pattern: &'a Pattern, adjacencies: &'a [&'a Adjacency],
            ...
            })
        {
            ...
            let approach_index = self.store.add_edge(
                ...
                ApproachWeight::ExtendStep(ExtendWeight {
                    extend_step,
                    target_vertex_rank,
                    adjacency_count: OrderedFloat::default(),
                    intersect_count: OrderedFloat::default(),
                }),
            );
...
impl Catalogue {
    ...
    pub fn set_pattern_count_with_index(&mut self, pattern_index: NodeIndex, count: OrderedFloat<f64>) {
            ...
            })
        {
            ...
            for extend_approach in extend_approaches {
                ...
                if extend_step.get_extend_edges_num() == 1 {
                    adjacency_count_map.insert(*extend_step.iter().next().unwrap(), target_pattern_count);
                    ...
                    extend_weight_mut.set_intersect_count(OrderedFloat::default());
                } else {
                    ...
                    let adjacency_count = extend_edges
                        ...
                        .sum();
                    let mut min_intersect_count = OrderedFloat::max_value();
    ...
    pub fn estimate_pattern_count(&self, pattern: &Pattern) -> OrderedFloat<f64> {
        let pattern_code = pattern.encode_to();
        ...
        if let Some(pattern_index) = self.get_pattern_index(&pattern_code) {
            ...
            let mut pattern_count_sum = OrderedFloat::default();
            ...
            for edge in pattern.edges_iter() {
                if let Some(sub_pattern) = pattern.clone().remove_edge(edge.get_id()) {
                    ...
                    if pattern.get_vertex_degree(edge.get_start_vertex().get_id()) > 1 {
                        ...
                        if start_vertex_count > OrderedFloat::default() {
                    ...
                    if pattern.get_vertex_degree(edge.get_end_vertex().get_id()) > 1 {
                        ...
                        if end_vertex_count > OrderedFloat::default() {
            ...
            if pattern_count_num > 0 {
                pattern_count_sum / pattern_count_num as f64
            } else {
                OrderedFloat::default()
            }
        }
    }

    pub fn estimate_pattern_count_random(
        &self, pattern: &Pattern, sample: usize, seed: u64,
    ) -> OrderedFloat<f64> {
        let mut rng = StdRng::seed_from_u64(seed);
        let counts = (0..sample)
            .map(|_| self.estimate_pattern_count_random_inner(pattern, &mut rng))
            .collect_vec();
        counts.iter().sum::<OrderedFloat<_>>() / OrderedFloat::from(counts.len() as f64)
    }

    fn estimate_pattern_count_random_inner(
        &self, pattern: &Pattern, rng: &mut StdRng,
    ) -> OrderedFloat<f64> {
        let pattern_code = pattern.encode_to();
        // If the pattern is stored in the catalog, directly return its count
        if let Some(pattern_index) = self.get_pattern_index(&pattern_code) {
            ...
            let mut sampled_pattern_count = None;
            while let Some(edge) = pattern.edges_iter().choose(rng) {
                if let Some(sub_pattern) = pattern.clone().remove_edge(edge.get_id()) {
                    let sub_pattern_count = self.estimate_pattern_count_random_inner(&sub_pattern, rng);
                    let edge_count = self.estimate_pattern_count_random_inner(
                        &Pattern::try_from(vec![edge.clone()]).unwrap(),
                        rng,
                    );
                    let mut pattern_count = sub_pattern_count * edge_count;
                    if pattern.get_vertex_degree(edge.get_start_vertex().get_id()) > 1 {
                        let start_vertex_count = self.estimate_pattern_count_random_inner(
                            &Pattern::from(edge.get_start_vertex()),
                            rng,
                        );
                        if start_vertex_count > OrderedFloat::default() {
                            pattern_count /= start_vertex_count;
                    ...
                    if pattern.get_vertex_degree(edge.get_end_vertex().get_id()) > 1 {
                        let end_vertex_count = self.estimate_pattern_count_random_inner(
                            &Pattern::from(edge.get_end_vertex()),
                            rng,
                        );
                        if end_vertex_count > OrderedFloat::default() {
                            pattern_count /= end_vertex_count;
                    ...
                    sampled_pattern_count = Some(pattern_count);
                    break
                }
            }
            sampled_pattern_count.unwrap_or_default()
        }
    }
}
...
fn get_vec_arranges<T: Clone>(vector: &[T], len: usize) -> Vec<Vec<T>> {
...
impl Display for Catalogue {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}", ron::ser::to_string_pretty(self, PrettyConfig::default()).unwrap())
    }
}
