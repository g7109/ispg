...
impl From<IrError> for FfiResult {
    fn from(err: IrError) -> Self {
        match err {
            ...
            _ => FfiResult::new(ResultCode::Others, "catalog error".to_string()),
        }
    }
}
...
pub(crate) fn cstr_to_expr_pb(cstr: *const c_char) -> Result<common_pb::Expression, FfiResult> {
    ...
    match str {
        Ok(s) => {
            let expr = str_to_expr_pb(s).map_err(IrError::from)?;
            Ok(expr)
        }
        Err(e) => Err(e),
    }
...
impl TryFrom<FfiProperty> for Option<common_pb::Property> {
    ...
    fn try_from(ffi: FfiProperty) -> Result<Self, FfiResult> {
        let result = match &ffi.opt {
            ...
            FfiPropertyOpt::Key => {
                let key: Option<_> = ffi.key.try_into()?;
                key.map(|key| common_pb::Property { item: Some(common_pb::property::Item::Key(key)) })
            }
        };
...
pub extern "C" fn boolean_as_const(boolean: bool) -> FfiConst {
    let data_type = FfiDataType::Boolean;
    FfiConst { data_type, boolean, ..Default::default() }
}

#[no_mangle]
pub extern "C" fn int32_as_const(int32: i32) -> FfiConst {
    let data_type = FfiDataType::I32;
    FfiConst { data_type, int32, ..Default::default() }
}

#[no_mangle]
pub extern "C" fn int64_as_const(int64: i64) -> FfiConst {
    let data_type = FfiDataType::I64;
    FfiConst { data_type, int64, ..Default::default() }
}

#[no_mangle]
pub extern "C" fn f64_as_const(float64: f64) -> FfiConst {
    let data_type = FfiDataType::F64;
    FfiConst { data_type, float64, ..Default::default() }
}

#[no_mangle]
pub extern "C" fn cstr_as_const(cstr: *const c_char) -> FfiConst {
    let data_type = FfiDataType::Str;
    FfiConst { data_type, cstr, ..Default::default() }
}
...
pub extern "C" fn init_logical_plan() -> *const c_void {
    let plan = Box::<LogicalPlan>::default();
...
mod params {
    ...
    pub extern "C" fn add_params_extra(
        ...
        if key.is_err() {
            ...
            if let Ok(val) = val {
                params.extra.insert(key.unwrap(), val);
            } else {
...
mod groupby {
    ...
    pub extern "C" fn add_groupby_key_alias(
        ...
        match (key_pb, alias_pb) {
            (Ok(key_pb), Ok(alias_pb)) => {
                group
                    .mappings
                    .push(pb::group_by::KeyAlias { key: Some(key_pb), alias: alias_pb });
            }
            (Err(e), _) => result = e,
            (_, Err(e)) => result = e,
        }
    ...
    pub extern "C" fn add_groupby_agg_fn(
        ...
        let alias_pb = alias.try_into();
        match (val_pb, alias_pb) {
            (Ok(val_pb), Ok(alias_pb)) => {
