...
impl Display for Node {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        if let Some(opr) = &self.opr.opr {
            write!(f, "ID: {}, Children: {:?}, ", self.id, self.children.iter().collect_vec())?;
            match opr {
                Opr::Project(op) => write!(f, "{:?}", op)?,
                Opr::Select(op) => write!(f, "{:?}", op)?,
                Opr::Join(op) => write!(f, "{:?}", op)?,
                Opr::Union(op) => write!(f, "{:?}", op)?,
                Opr::GroupBy(op) => write!(f, "{:?}", op)?,
                Opr::OrderBy(op) => write!(f, "{:?}", op)?,
                Opr::Dedup(op) => write!(f, "{:?}", op)?,
                Opr::Unfold(op) => write!(f, "{:?}", op)?,
                Opr::Apply(op) => write!(f, "{:?}", op)?,
                Opr::SegApply(op) => write!(f, "{:?}", op)?,
                Opr::Scan(op) => write!(f, "{:?}", op)?,
                Opr::Limit(op) => write!(f, "{:?}", op)?,
                Opr::Auxilia(op) => write!(f, "{:?}", op)?,
                Opr::As(op) => write!(f, "{:?}", op)?,
                Opr::Intersect(op) => write!(f, "{:?}", op)?,
                Opr::Sink(op) => write!(f, "{:?}", op)?,
                Opr::Fused(op) => write!(f, "{:?}", op)?,
                Opr::Vertex(op) => write!(f, "{:?}", op)?,
                Opr::Edge(op) => write!(f, "{:?}", op)?,
                Opr::Path(op) => write!(f, "{:?}", op)?,
                Opr::PathStart(op) => write!(f, "{:?}", op)?,
                Opr::PathEnd(op) => write!(f, "{:?}", op)?,
                Opr::Pattern(op) => write!(f, "{:?}", op)?,
                Opr::ExpandIntersect(op) => write!(f, "{:?}", op)?,
            }
        }
        Ok(())
    }
    ...
    fn try_from(pb: pb::LogicalPlan) -> Result<Self, Self::Error> {
        ...
        for (id, node) in nodes_pb.into_iter().enumerate() {
            if let Some(mut opr) = node.opr {
                if let Some(pb::logical_plan::operator::Opr::Apply(apply)) = opr.opr.as_mut() {
...
impl From<LogicalPlan> for pb::LogicalPlan {
    fn from(plan: LogicalPlan) -> Self {
        ...
        for (_, node) in &plan.nodes {
            ...
            if let Some(pb::logical_plan::operator::Opr::Apply(apply)) = operator.opr.as_mut() {
                apply.subtask = id_map[&(apply.subtask as NodeId)] as PbNodeId;
            }
    ...
    pub fn append_operator_as_node(
        ...
        let new_curr_node_rst = match opr.opr.as_ref().unwrap() {
            Opr::Pattern(pb_pattern) => {
                if parent_ids.len() == 1 {
                    if let Ok(mut plan) = ExtendStrategy::init(pb_pattern, &mut self.meta)
            ...
            Some(pb::logical_plan::operator::Opr::Apply(apply_opr)) => {
                if let Some(from_node) = self.get_node(apply_opr.subtask as NodeId) {
                    ...
                    let mut subplan = if from_node == curr_node {
                        let p = LogicalPlan { meta: self.meta.clone(), ..Default::default() };
                        Some(p)
                    } else {
...
fn is_join(pb_plan: &pb::LogicalPlan) -> bool {
    for node in pb_plan.nodes.iter() {
        if let Some(oprator) = node.opr.as_ref() {
            if let Some(pb::logical_plan::operator::Opr::Join(_)) = oprator.opr.as_ref() {
                return true;
            }
        }
    }
...
fn triplet_to_index_predicate(
    ...
    let mut is_eq = false;
    let mut value = None;
    if let Some(common_pb::expr_opr::Item::Var(var)) = &operators.first().unwrap().item {
        if let Some(property) = &var.property {
            if let Some(common_pb::property::Item::Key(col)) = &property.item {
                let (is_pk, num_pks) = check_primary_key_from_pb(schema, table, col, is_vertex);
                if is_pk && num_pks == 1 {
    ...
    if let Some(common_pb::expr_opr::Item::Logical(l)) = &operators.get(1).unwrap().item {
        if *l == 0 {
            // Eq
            is_eq = true;
        }
    ...
    if let Some(common_pb::expr_opr::Item::Const(c)) = &operators.get(2).unwrap().item {
        value = Some(c.clone());
    }
    if value.is_none() {
        return Ok(None);
    ...
    fn preprocess(&mut self, meta: &StoreMeta, plan_meta: &mut PlanMeta) -> IrResult<()> {
        ...
        if let Some(params) = self.params.as_mut() {
            if self.idx_predicate.is_none() {
                if let Some(table) = params.tables.first() {
...
impl AsLogical for pb::Apply {
    fn preprocess(&mut self, _meta: &StoreMeta, plan_meta: &mut PlanMeta) -> IrResult<()> {
        ...
        if let Some(alias) = self.alias.as_mut() {
            let tag_id = get_or_set_tag_id(alias, plan_meta)?;
            plan_meta.set_tag_nodes(tag_id, vec![plan_meta.get_curr_node()]);
        } else if self.join_kind != 4 && self.join_kind != 5 {
...
mod test {
    ...
    fn preprocess_expr() {
        ...
        preprocess_expression(&mut expression, &meta, &mut plan_meta, false).unwrap();
        let opr = expression.operators.first().unwrap().clone();
        match opr.item.unwrap() {
            common_pb::expr_opr::Item::Var(var) => {
