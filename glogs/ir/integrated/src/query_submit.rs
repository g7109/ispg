...
pub fn submit_query_and_fetch_results(
    job_req: JobRequest, conf: JobConf,
            ...
            let tag: Option<KeyId> = if let Some(tag) = column.name_or_id {
                match tag.item.unwrap() {
                    common_pb::name_or_id::Item::Name(name) => {
                        Some(name.parse::<KeyId>().unwrap_or(KeyId::MAX))
                    }
...
pub fn split_intermediate_pb_logical_plan(pb_plan: &pb::LogicalPlan) -> Vec<pb::LogicalPlan> {
    ...
    let mut intermediate_pb_plan: pb::LogicalPlan = pb::LogicalPlan { roots: vec![], ..Default::default() };
    let pb_plan_len = pb_plan.nodes.len();
