...
mod test {
    ...
    fn execute_pb_logical_plan(pb_plan: pb::LogicalPlan) {
        ...
        let results = submit_query(request, number_of_cores);
        for result in results.flatten() {
            if let Some(record) = parse_result(result) {
                println!("{:?}", record);
            }
        }
    ...
    fn build_join_opr(left_keys: Vec<i32>, right_keys: Vec<i32>) -> logical_plan::Operator {
        let join_keys_left: Vec<common_pb::Variable> = left_keys
            .into_iter()
            .map(|join_id| common_pb::Variable { tag: Some(join_id.into()), property: None })
            .collect();
        ...
        let pb_plan = pb::LogicalPlan {
            nodes: vec![
                ...
                pb::logical_plan::Node {
                    opr: Some(pb::Intersect { parents: vec![], key: Some(2_i32.into()) }.into()),
                    children: vec![11],
                },
