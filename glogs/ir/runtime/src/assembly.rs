...
use crate::process::operator::shuffle::{RecordRouter, RoundRobinRecordRouter};
...
impl FnGenerator {
    ...
    fn gen_source(&self, opr: AlgebraOpr) -> FnGenResult<DynIter<Record>> {
        ...
        source_opr.gen_source(worker_id.index as usize)
    }

    fn gen_shuffle(&self, _res: &BinaryResource) -> FnGenResult<RecordShuffle> {
        ...
        let record_router = RoundRobinRecordRouter::new(num_workers)?;
