    ...
    fn exec(&self, mut input: Record) -> FnResult<Option<Record>> {
        if self.is_append {
            if self.projected_columns.len() == 1 {
                let (projector, alias) = self.projected_columns.first().unwrap();
                let entry = exec_projector(&input, projector)?;
                if entry.is_none() {
                    Ok(None)
                } else {
                    input.append_arc_entry(entry, *alias);
                    Ok(Some(input))
                }
            } else {
            ...
            if self.projected_columns.len() == 1 {
                ...
                if entry.is_none() {
                    ...
                    new_record.append_arc_entry(entry, *alias);
                    Ok(Some(new_record))
                }
            } else {
                for (projector, alias) in self.projected_columns.iter() {
...
mod tests {
    ...
    fn project_multi_tag_multi_mapping() {
        ...
        let mut r1 = Record::new(v1, Some(TAG_A));
        r1.append(v3.clone(), Some(TAG_B));
        let mut r2 = Record::new(v2, Some(TAG_A));
        r2.append(v3.clone(), Some(TAG_B));
    ...
    fn project_error_test() {
        ...
        let r1 = Record::new(common_obj1, Some(TAG_A));
        let r2 = Record::new(common_obj2, Some(TAG_A));
        let source = vec![r1, r2];
        let project_opr_pb = pb::Project {
            mappings: vec![pb::project::ExprAlias {
