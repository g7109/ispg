...
pub(crate) mod tests {
    ...
    fn init_record() -> Record {
        ...
        record.append(vertex2, Some(0 as KeyId));
        record.append(object3, Some(1 as KeyId));
        record
    }
    ...
    pub fn init_source_with_tag() -> Vec<Record> {
        let v1 = init_vertex1();
        let v2 = init_vertex2();
        let r1 = Record::new(v1, Some(TAG_A));
        let r2 = Record::new(v2, Some(TAG_A));
        vec![r1, r2]
    }
    ...
    fn test_get_tag_entry() {
        let tag_key = TagKey { tag: Some(0 as KeyId), key: None };
    ...
    fn test_get_tag_key_entry() {
        let tag_key = TagKey { tag: Some(0 as KeyId), key: Some(PropKey::Key("age".into())) };
        let expected = 27;
        let record = init_record();
        let entry = tag_key.get_arc_entry(&record).unwrap();
        match entry.as_ref() {
