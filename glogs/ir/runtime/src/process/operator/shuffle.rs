...
pub struct RoundRobinRecordRouter {
    num_workers: usize,
    next_worker_id: Cell<u64>,
}

impl RouteFunction<Record> for RoundRobinRecordRouter {
    fn route(&self, _: &Record) -> FnResult<u64> {
        let worker_id = self.next_worker_id.get();
        self.next_worker_id
            .set((worker_id + 1) % (self.num_workers as u64));
        Ok(worker_id)
    }
}

impl RoundRobinRecordRouter {
    pub fn new(num_workers: usize) -> Result<Self, ParsePbError> {
        Ok(Self { num_workers, next_worker_id: Cell::new(0) })
    }
}
...
impl RouteFunction<Record> for RecordRouter {
    fn route(&self, t: &Record) -> FnResult<u64> {
        if let Some(entry) = t.get(self.shuffle_key) {
