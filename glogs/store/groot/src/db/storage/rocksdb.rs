...
use rocksdb::{DBCompactionStyle, DBCompressionType, Env, WriteBatch};
...
impl ExternalStorage for RocksDB {
    ...
    fn open_backup_engine(&self, backup_path: &str) -> GraphResult<Box<dyn ExternalStorageBackup>> {
        let backup_opts = BackupEngineOptions::new(backup_path).map_err(|e| {
            let msg = format!(
                "create rocksdb backup engine options at {} failed, because {}",
                backup_path.to_string(),
                e.into_string()
            );
            gen_graph_err!(GraphErrorCode::ExternalStorageError, msg)
        })?;
        let backup_env = Env::new().map_err(|e| {
            let msg = format!(
                "create rocksdb backup engine env at {} failed, because {}",
                backup_path.to_string(),
                e.into_string()
            );
            gen_graph_err!(GraphErrorCode::ExternalStorageError, msg)
        })?;
        let backup_engine = BackupEngine::open(&backup_opts, &backup_env).map_err(|e| {
            let msg = format!(
