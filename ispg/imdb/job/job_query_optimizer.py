...
from ispg.core.estimator import GLogsEstimator, SchemaInfo, load_imdb_schema  # noqa: E402
...
# -----------------------------------------------------
# Cost-model α coefficients (§5.3 / §5.4) — kept aligned with the LDBC
# optimizer (ispg/ldbc/ic/optimizer.py) so both back-ends charge an edge
# closed inside the matching at the cheaper EdgeCheck rate.
# -----------------------------------------------------
ALPHA_EXP = 1.0   # Expand    : α_exp · F(Ut)
ALPHA_CHK = 0.1   # EdgeCheck : α_chk · F(Us)  (constant-time membership test)
...
class JobQueryOptimizer:
    ...
    def build_plan(
        ...
        for e_idx in seq:
            ...
            edge_spec = self.edges_spec[e_idx]
            reverse = False
            # An edge whose both endpoints are already bound closes a cycle and
            # is tested inside the matching by an EdgeCheck (§5.4 ExpandInt),
            # introducing no new vertex; otherwise it is a navigating Expand.
            is_check = edge_spec.src_tag in visited and edge_spec.dst_tag in visited
            ...
            if is_check:
                # EdgeCheck charged on its input state F(Us) at the α_chk rate
                # (constant-time membership test, §5.4/§5.6); no new vertex.
                F_in = self.subset_F.get(prev_mask, self.vertex_F_eff[anchor_tag])
                steps.append(
                    PlanStep(
                        step_id=step_id,
                        op="EDGECHECK",
                        pattern=pattern_name,
                        cost=self.cost_scale * ALPHA_CHK * F_in,
                        detail=f"close {src_alias}->{dst_alias} ({edge_spec.description})",
                        from_var=src_alias,
                        to_var=dst_alias,
                    )
                )
...
def load_schema(repo_root: Path) -> SchemaInfo:
    return load_imdb_schema(repo_root)
